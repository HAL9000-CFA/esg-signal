...
Examples:
  ...
  python agent1_cli.py --ticker TSLA --start 20240101 --end 20240331 --email your@email.com
    ...
    parser.add_argument(
        '--email',
        required=True,
        help='Your email address (required for SEC EDGAR API compliance)'
    )
    
    # Optional arguments
    parser.add_argument(
        '--company',
        help='Full company name (for Companies House and other non-ticker sources)'
    )
    
    parser.add_argument(
        '--pdf',
        help='Path to sustainability report PDF for extraction'
    )
    ...
    parser.add_argument(
        '--start',
        help='Start date for GDELT search (YYYYMMDD format)'
    )
    
    parser.add_argument(
        '--end',
        help='End date for GDELT search (YYYYMMDD format)'
    )
    
    parser.add_argument(
        '--output',
        help='Output file path (default: agent1_TICKER_TIMESTAMP.json)'
    )
    ...
    # initialize Agent 1
    print(f"\n{'='*70}")
    print(f"  Agent 1: Multi-Source ESG Data Fetcher")
    print(f"{'='*70}\n")
